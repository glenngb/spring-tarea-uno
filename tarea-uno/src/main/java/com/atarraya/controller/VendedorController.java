...
import com.atarraya.service.IVendedorService;
...
public class VendedorController {
		...
		@PostMapping
		public ResponseEntity<Object> registrar(@Valid @RequestBody Vendedor especialidad) {
			Vendedor obj = service.registrar(especialidad);
			
			URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(especialidad.getIdVendedor()).toUri();
			return ResponseEntity.created(location).build();
		}
		
		@PutMapping
		public ResponseEntity<Vendedor> modificar(@Valid @RequestBody Vendedor especialidad) {
			Vendedor obj = service.modificar(especialidad);
			return new ResponseEntity<Vendedor>(obj, HttpStatus.OK);
		}
