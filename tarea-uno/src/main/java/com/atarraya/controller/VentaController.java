...
public class VentaController {
	...
	@PostMapping
	public ResponseEntity<Object> registrar(@Valid @RequestBody Venta especialidad) {
		Venta obj = service.registrar(especialidad);
		
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(especialidad.getIdVenta()).toUri();
		return ResponseEntity.created(location).build();
	}
	
	@PutMapping
	public ResponseEntity<Venta> modificar(@Valid @RequestBody Venta especialidad) {
		Venta obj = service.modificar(especialidad);
		return new ResponseEntity<Venta>(obj, HttpStatus.OK);
	}
