...
public class MarcaController {
	...
	@PostMapping
	public ResponseEntity<Object> registrar(@Valid @RequestBody Marca especialidad) {
		Marca obj = service.registrar(especialidad);
		
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(especialidad.getIdMarca()).toUri();
		return ResponseEntity.created(location).build();
	}
	
	@PutMapping
	public ResponseEntity<Marca> modificar(@Valid @RequestBody Marca especialidad) {
		Marca obj = service.modificar(especialidad);
		return new ResponseEntity<Marca>(obj, HttpStatus.OK);
	}
